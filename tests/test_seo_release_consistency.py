...
def test_source_allowlist_rejects_auth_and_other_modules() -> None:
    assert source_path_allowed("app/seo_demo_fixture_loader.py")
    ...
    assert source_path_allowed("migrations/versions/20260913_0099_geo_review_audit.py")
    assert source_path_allowed("tests/test_geo_review_audit_migration.py")
    assert not source_path_allowed("migrations/versions/20260913_0099_unreviewed_geo_change.py")
    assert not source_path_allowed("tests/test_unreviewed_geo_migration.py")
...
def test_seo_dashboard_shows_tenant_automation_run_status() -> None:
    ...
    assert "await fetchSeoAutomationRuns({ tenantId: requestedTenantId, siteId: requestedSiteId" in dashboard
    ...
    assert "手动重跑" in dashboard
    assert "session.canEdit('seo.dashboard')" in dashboard
    assert "不调用 AI、不发布内容、不修改客户网站" in dashboard
    assert "操作人 #" in dashboard
    assert "客户或网站已切换，请重新确认" in dashboard
    assert "requestedSiteName" in dashboard
