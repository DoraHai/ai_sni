...
def test_sem_shell_keeps_existing_identity_and_dashboard_entry():
    app = _source("frontend/src/App.vue")
    router = _source("frontend/src/router/index.js")

    assert "SEM 智投平台" in app
    assert "平台门户" in app
    assert "G-Snipers 获客指挥台" not in app
    assert "redirect: '/monitor/dashboard'" in router
