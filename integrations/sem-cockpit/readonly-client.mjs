...
function validateReportAssociation(association, item, accountId) {
  contract(object(association) && ['matched', 'account_mismatch', 'no_report', 'ownership_unknown', 'report_ownership_unknown'].includes(association.status))
  ...
  contract(typeof association.has_unassigned_reports === 'boolean')
  ...
  contract(association.other_observed_account_ids.every(id => positive(id) && id !== item.baidu_account_id))
  contract((association.status === 'ownership_unknown') === (item.baidu_account_id === null))
  if (association.status === 'matched') {
    ...
    if (item.baidu_account_id !== null) {
      ...
      contract((association.status === 'report_ownership_unknown') ===
        (association.other_observed_account_ids.length === 0 && association.has_unassigned_reports))
      contract((association.status === 'no_report') ===
        (association.other_observed_account_ids.length === 0 && !association.has_unassigned_reports))
    }
  }
  if (accountId !== undefined) contract(association.status !== 'account_mismatch' &&
    association.status !== 'report_ownership_unknown' && !association.has_unassigned_reports)
}
...
function validateKeywords(data, params) {
  ...
  const associationCounts = { matched: 0, account_mismatch: 0, no_report: 0, ownership_unknown: 0, report_ownership_unknown: 0 }
  ...
  contract(Object.keys(data.association_summary.counts).length === 5)
  for (const status of ['matched', 'account_mismatch', 'no_report', 'ownership_unknown', 'report_ownership_unknown']) {
