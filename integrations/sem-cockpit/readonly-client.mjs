...
function validateReportAssociation(association, item, accountId) {
  const legacyStatuses = ['matched', 'account_mismatch', 'no_report']
  const evidenceStatuses = [...legacyStatuses, 'ownership_unknown', 'report_ownership_unknown']
  contract(object(association) && evidenceStatuses.includes(association.status))
  ...
  const compatibilityEvidence = Object.hasOwn(association, 'evidence_status') ||
    Object.hasOwn(association, 'observed_known_account_ids')
  const additiveEvidence = compatibilityEvidence || Object.hasOwn(association, 'has_unassigned_reports')
  if (Object.hasOwn(association, 'evidence_status')) contract(evidenceStatuses.includes(association.evidence_status))
  if (Object.hasOwn(association, 'has_unassigned_reports')) contract(typeof association.has_unassigned_reports === 'boolean')
  ...
  contract(association.other_observed_account_ids.every(id =>
    (positive(id) || (!additiveEvidence && id === null)) && id !== item.baidu_account_id))
  const inferredUnassigned = association.other_observed_account_ids.includes(null)
  const hasUnassignedReports = association.has_unassigned_reports ?? inferredUnassigned
  contract(!inferredUnassigned || hasUnassignedReports)
  const observedKnownAccountIds = association.observed_known_account_ids ??
    association.other_observed_account_ids.filter(id => id !== null)
  contract(Array.isArray(observedKnownAccountIds) &&
    new Set(observedKnownAccountIds).size === observedKnownAccountIds.length &&
    observedKnownAccountIds.every(id => positive(id) && id !== item.baidu_account_id))
  const evidenceStatus = association.evidence_status ?? (
    evidenceStatuses.includes(association.status) && !legacyStatuses.includes(association.status)
      ? association.status
      : item.baidu_account_id === null ? 'ownership_unknown'
        : association.status === 'matched' ? 'matched'
          : observedKnownAccountIds.length ? 'account_mismatch'
            : hasUnassignedReports ? 'report_ownership_unknown' : 'no_report'
  )
  const expectedLegacyStatus = evidenceStatus === 'matched' || evidenceStatus === 'account_mismatch'
    ? evidenceStatus : 'no_report'
  const legacyStatus = legacyStatuses.includes(association.status) ? association.status : expectedLegacyStatus
  contract((evidenceStatus === 'ownership_unknown') === (item.baidu_account_id === null))
  const unsafeLegacyNullMatch = !additiveEvidence && item.baidu_account_id === null && association.status === 'matched'
  if (unsafeLegacyNullMatch) {
    contract(association.matched_report_groups > 0 && item.coverage.status === 'observed')
  } else if (evidenceStatus === 'matched') {
    contract(association.matched_report_groups > 0 && association.matched_report_groups === item.coverage.observed_days)
    contract(item.coverage.status === 'observed')
  } else {
    contract(association.matched_report_groups === 0 && item.coverage.status === 'no_data' && allNull(item.metrics))
    contract(item.phone_button_clicks.status === 'no_data')
    if (item.baidu_account_id !== null) {
      contract((evidenceStatus === 'account_mismatch') === (observedKnownAccountIds.length > 0))
      contract((evidenceStatus === 'report_ownership_unknown') ===
        (observedKnownAccountIds.length === 0 && hasUnassignedReports))
      contract((evidenceStatus === 'no_report') ===
        (observedKnownAccountIds.length === 0 && !hasUnassignedReports))
    }
  }
  if (legacyStatuses.includes(association.status)) {
    if (compatibilityEvidence) {
      contract(association.status === expectedLegacyStatus)
      contract(association.status !== 'matched' || association.other_observed_account_ids.length === 0)
      contract((association.status === 'account_mismatch') ===
        (association.other_observed_account_ids.length > 0))
      if (association.status === 'account_mismatch') {
        contract(JSON.stringify(association.other_observed_account_ids) === JSON.stringify(observedKnownAccountIds))
      }
    } else if (!Object.hasOwn(association, 'has_unassigned_reports')) {
      contract((association.status === 'account_mismatch') ===
        (association.other_observed_account_ids.length > 0))
    }
  }
  if (accountId !== undefined) contract(evidenceStatus !== 'account_mismatch' &&
    evidenceStatus !== 'report_ownership_unknown' && !hasUnassignedReports)
  return { legacyStatus, evidenceStatus }
}
...
function validateKeywords(data, params) {
  ...
  const evidenceCounts = { matched: 0, account_mismatch: 0, no_report: 0, ownership_unknown: 0, report_ownership_unknown: 0 }
  for (const item of data.items) {
    ...
    const association = validateReportAssociation(item.report_association, item, params.baidu_account_id)
    associationCounts[association.legacyStatus]++
    evidenceCounts[association.evidenceStatus]++
  }
  ...
  const countKeys = Object.keys(data.association_summary.counts)
  contract(countKeys.length === 3 || countKeys.length === 5)
  const expectedCounts = countKeys.length === 3 ? associationCounts : evidenceCounts
  for (const status of Object.keys(expectedCounts)) {
    contract(data.association_summary.counts[status] === expectedCounts[status])
  }
  if (Object.hasOwn(data.association_summary, 'evidence_counts')) {
    contract(object(data.association_summary.evidence_counts) &&
      Object.keys(data.association_summary.evidence_counts).length === 5)
    for (const status of Object.keys(evidenceCounts)) {
      contract(data.association_summary.evidence_counts[status] === evidenceCounts[status])
    }
  }
}
