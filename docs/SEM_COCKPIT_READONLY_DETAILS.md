...
- 关键词资产的 `asset_updated_at` 与日报 `coverage.updated_at` 分离。
  ...
  `account_mismatch` 表示窗口内只有其他账户桶（含未归属桶）的同ID报告，`ownership_unknown` 表示关键词资产本身没有账户归属。
  后两种状态的指标与电话点击均保持无数据，不把两个 `baidu_account_id=null` 当作同一账户。
  `association_summary` 只统计当前页四种状态，`completeness=unknown`，用于避免 `keywords.baidu_account_id` 为空或漂移时静默显示成真实零值。
