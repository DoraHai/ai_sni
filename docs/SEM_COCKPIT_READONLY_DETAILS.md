...
- 关键词资产的 `asset_updated_at` 与日报 `coverage.updated_at` 分离。
  列表逐项返回 `report_association`。兼容字段 `status` 保持 `matched/account_mismatch/no_report` 三态，
  新字段 `evidence_status` 提供严格五态：`matched` 表示相同非空账户与关键词ID有报告，`no_report` 表示窗口内没有同ID报告，
  `account_mismatch` 表示存在已知且不同账户的同ID报告，`report_ownership_unknown` 表示只有归属未知的同ID报告，
  `ownership_unknown` 表示关键词资产本身没有账户归属。`has_unassigned_reports` 独立保留报告侧未知归属证据，因此已知异账户和未知报告同时存在时不会丢失后者。
  未归属报告不参与指标或电话点击聚合，不把两个 `baidu_account_id=null` 当作同一账户。
  `association_summary.counts` 保持旧客户端需要的三键当前页统计，`evidence_counts` 提供五态当前页统计；`completeness=unknown`。
  新客户端同时接受旧三态响应、已部署过渡期五态响应和上述兼容响应，避免前后端滚动发布时整块丢弃关键词数据。
