...
### 2.3 当前进度快照（2026-08-06）

| 项 | 状态 |
| --- | --- |
| `main` tip | 含 Vue 编辑器切片、事实/补丁/Brief 修复、PR #11～#22 |
| P1 内容工作台 Vue | ✅ 任务列表 + 母稿编辑器（渠道/审校/回填） |
| P2 真多引擎采样 | ✅ `openai_compat` 可配 |
| 自助 MVP 主环 | **接近可交付** · 清单 `docs/GEO_DELIVERY_CHECKLIST.md` |
| 验收脚本 | `accept_geo_m1.py` + **`accept_geo_delivery.py`（内容主环）** |
| 下一波（产品化加深） | P3 社交 OAuth / P4 交付包 / E2E · 见 §2.4 |
...
| 路径 | 页面 | 权限 |
| --- | --- | --- |
| `/geo/overview` | GEO 概览（KPI + CSV） | `geo.content` |
| `/geo/visibility` | AI 可见度 | `geo.content` |
| `/geo/visibility/snapshots` | 采集与判断 | `geo.content` |
| `/geo/citations` | AI 引用次数 | `geo.content` |
| `/geo/competitors` | 竞品分析 | `geo.content` |
| `/geo/brand` | 品牌资料（原 `/geo/businesses` 已下线） | `geo.content` |
| `/geo/knowledge` | 知识库 | `geo.content` |
| `/geo/tasks` | 优化文章列表 | `geo.content` |
| `/geo/tasks/:taskId` | 内容编辑器 | `geo.content` |
| `/geo/questions` | 优化意图词 | `geo.content` |
...
| `docs/GEO_CODE_DELIVERY.md` | **无生产机 · 仅 GEO 代码交付**边界与验收 |
| `docs/GEO_PRODUCTION_RUNBOOK.md` | 真实服务器上线（有机器时后置） |
