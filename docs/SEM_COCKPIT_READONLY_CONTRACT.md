...
> 本文记录 PR #374 基础交付。后续 B 包扩展电话按钮点击、关键词、详情维度和搜索词，
> 并提供工作台消费适配；当前能力请同时阅读 [B 包契约及验收](SEM_COCKPIT_READONLY_DETAILS.md)。
> A基础看板仍不返回电话按钮点击；B仅在关键词列表/详情返回已有原始字段的覆盖结果。
> 下文“未增加关键词日期列表”为基础版本状态，由 B 包新增路径补充，旧接口行为不变。
