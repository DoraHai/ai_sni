...
import { downloadSeoPublishedLinksTemplate,fetchSeoContentAssets,importSeoPublishedLinks,updateSeoContentAsset } from '../../api/seo'
import { currentTenantId,session } from '../../store/session'

const loading=ref(false),saving=ref(false),error=ref(''),dialog=ref(false),items=ref([]),query=ref(''),filter=ref('全部')
const importInput=ref(null),importDialog=ref(false),importing=ref(false),importFile=ref(null),importResult=ref(null)
...
function selectImport(){if(!currentTenantId.value)return ElMessage.warning('请先选择客户');importInput.value?.click()}
async function previewImport(event){const file=event.target.files?.[0];event.target.value='';if(!file)return;if(!file.name.toLowerCase().endsWith('.xlsx'))return ElMessage.warning('请选择 .xlsx 格式的 Excel 文件');importFile.value=file;importing.value=true;try{importResult.value=await importSeoPublishedLinks({tenantId:currentTenantId.value,file,dryRun:true});importDialog.value=true}catch(e){ElMessage.error(e.message)}finally{importing.value=false}}
async function commitImport(){if(!importFile.value||!importResult.value||importResult.value.failed)return;importing.value=true;try{const result=await importSeoPublishedLinks({tenantId:currentTenantId.value,file:importFile.value,dryRun:false});importResult.value=result;if(result.committed){ElMessage.success(`已批量登记 ${result.imported} 条发布链接`);importDialog.value=false;importFile.value=null;await load()}else{ElMessage.warning('文件内容已变化，请检查后重试')}}catch(e){ElMessage.error(e.message)}finally{importing.value=false}}
async function downloadTemplate(){try{const blob=await downloadSeoPublishedLinksTemplate();const url=URL.createObjectURL(blob);const anchor=document.createElement('a');anchor.href=url;anchor.download='SEO发布链接批量登记模板.xlsx';anchor.click();URL.revokeObjectURL(url)}catch(e){ElMessage.error(e.message)}}
...
<template>
  <div class="distribution-page" v-loading="loading">
    <section class="distribution-hero">
      ...
      <div v-if="canEdit" class="hero-actions"><input ref="importInput" class="file-input" type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" @change="previewImport"><button class="btn" :disabled="importing" @click="selectImport">Excel 批量登记</button><button class="btn primary" @click="open()">＋ 登记发布</button></div>
    </section>
    ...
    <el-dialog v-model="importDialog" title="Excel 批量登记预检" width="900px">
      <div v-if="importResult" class="import-preview">
        ...
        <el-alert v-else :title="`预检通过：${importResult.valid} 条记录可以安全导入。`" type="success" :closable="false" show-icon/>
        ...
        <el-table :data="importResult.rows" max-height="400" size="small" border>
          <el-table-column prop="row_number" label="行" width="58"/><el-table-column prop="content_id" label="资产ID" width="88"/><el-table-column prop="title" label="内容标题" min-width="170" show-overflow-tooltip/><el-table-column prop="platform" label="平台" width="105" show-overflow-tooltip/><el-table-column prop="page_url" label="发布链接" min-width="210" show-overflow-tooltip/><el-table-column prop="action" label="写入方式" width="105"/><el-table-column label="检查结果" min-width="170"><template #default="scope"><span v-if="scope.row.status==='valid'" class="row-valid">通过</span><span v-else class="row-error">{{scope.row.errors.join('；')}}</span></template></el-table-column>
        </el-table>
        <p class="import-help">支持前 1000 行；用“内容资产ID”精确匹配，或用唯一的“内容标题”匹配。发布链接仅校验格式，不会访问外部网站。<button @click="downloadTemplate">下载 Excel 模板</button></p>
      </div>
