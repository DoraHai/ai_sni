...
import { fetchSeoImageRemediationWorkbench, saveSeoImageRemediation } from '../../api/seo'
...
<template>
  <section class="image-workbench">
    <header>
      ...
      <div class="header-actions"><el-button :loading="exporting" :disabled="!stats.informative_approved_count" @click="exportApproved">导出已审核整改</el-button><el-button v-if="canEdit" type="primary" :loading="approving" :disabled="!eligible.length" @click="batchApprove">批量审核通过（{{ eligible.length }}）</el-button></div>
    </header>
    ...
    <el-table v-loading="loading" :data="result.items" empty-text="当前筛选下没有图片整改证据" @selection-change="selected = $event">
      <el-table-column type="selection" width="44" :selectable="isEligible"/>
    ...
    <footer><span>批量通过只处理已填写完整的草稿；未判断图片必须进入页面逐图确认。</span><el-pagination v-model:current-page="page" v-model:page-size="pageSize" :page-sizes="[25,50,100]" :total="result.total" layout="total, sizes, prev, pager, next" @current-change="load" @size-change="page = 1; load()"/></footer>
