...
const flags = reactive({ unanswered: false, demandFirst: false })
...
const groups = computed(() => (result.value?.groups || []).map(group => ({ ...group, intents: group.intents.map(intent => ({ ...intent,
  questions: intent.questions.filter(q => (!flags.unanswered || !q.answer_count) && (!query.value.trim() || `${q.title} ${q.topic}`.toLowerCase().includes(query.value.trim().toLowerCase()))).sort((a,b)=>flags.demandFirst ? demandPriority(b)-demandPriority(a) || (b.relevance||0)-(a.relevance||0) : 0),
