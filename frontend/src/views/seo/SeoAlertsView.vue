...
<template>
  <div class="seo-suite" v-loading="loading">
    ...
    <section class="suite-panel">
      ...
      <ul v-if="items.length" class="suite-list"><li v-for="item in items" :key="`${item.type}-${item.object_id}`" tabindex="0" role="button" @click="openDetail(item)" @keydown.enter="openDetail(item)"><span class="suite-pill" :class="item.severity==='high'?'danger':'warn'">{{item.severity==='high'?'P0':'P1'}}</span><div class="grow"><b>{{item.title}}</b><small>{{item.detail}} · {{labels[item.type]}}</small></div><time>{{item.occurred_at?new Date(item.occurred_at).toLocaleString():'—'}}</time><b class="detail-arrow">查看详情 →</b></li></ul>
