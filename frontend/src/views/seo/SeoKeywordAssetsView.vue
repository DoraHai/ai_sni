...
const form=reactive({keyword:'',cluster:'',intent:'',monthly_volume:null,difficulty:null,priority:'P2',landing_page:'',status:'active',notes:''})
...
function open(row=null){editing.value=row;Object.assign(form,{keyword:row?.keyword||'',cluster:row?.cluster||'',intent:row?.intent||'',monthly_volume:row?.monthly_volume??null,difficulty:row?.difficulty??null,priority:row?.priority||'P2',landing_page:row?.landing_page||'',status:row?.status||'active',notes:row?.notes||''});dialog.value=true}
async function load(){if(!currentTenantId.value){error.value='请先选择客户';result.value={items:[],total:0,stats:{}};return}loading.value=true;try{result.value=await fetchSeoKeywords({tenantId:currentTenantId.value,q:query.value,status:'',pageSize:200});error.value=''}catch(e){error.value=e.message}finally{loading.value=false}}
async function save(){if(!form.keyword.trim())return ElMessage.warning('请填写关键词');saving.value=true;try{const payload={cluster:form.cluster||null,intent:form.intent||null,monthly_volume:form.monthly_volume,difficulty:form.difficulty,priority:form.priority,landing_page:form.landing_page||null,status:form.status,notes:form.notes||null};if(editing.value)await updateSeoKeyword({keywordId:editing.value.id,tenantId:currentTenantId.value,payload});else await createSeoKeyword({tenant_id:currentTenantId.value,keyword:form.keyword.trim(),...payload});dialog.value=false;ElMessage.success('关键词已保存');await load()}catch(e){ElMessage.error(e.message)}finally{saving.value=false}}
async function importRows(){const lines=importText.value.split(/\r?\n/).map(v=>v.trim()).filter(Boolean);if(!lines.length)return ElMessage.warning('请填写关键词');saving.value=true;try{const items=lines.map(line=>{const [keyword,cluster,intent,volume,difficulty,priority,landing]=line.split(/\t|,/).map(v=>v?.trim());return{tenant_id:currentTenantId.value,keyword,cluster:cluster||null,intent:intent||null,monthly_volume:volume?Number(volume):null,difficulty:difficulty?Number(difficulty):null,priority:['P0','P1','P2','P3'].includes(priority)?priority:'P2',landing_page:landing||null}});await importSeoKeywords({tenant_id:currentTenantId.value,items});importOpen.value=false;importText.value='';ElMessage.success('关键词已导入');await load()}catch(e){ElMessage.error(e.message)}finally{saving.value=false}}
function toggleStatus(row){editing.value=row;Object.assign(form,{...row,status:row.status==='active'?'paused':'active'});save()}
let timer;watch(query,()=>{clearTimeout(timer);timer=setTimeout(load,260)});watch(currentTenantId,load);onMounted(load)
</script>
...
<template>
  <div class="manage-page" v-loading="loading">
    <header class="manage-topbar">
      ...
      <div class="top-actions"><label class="global-search"><span>⌕</span><input v-model="query" placeholder="搜索关键词…"></label><button v-if="canEdit" class="ui-btn" @click="importOpen=true">批量导入</button><button v-if="canEdit" class="ui-btn primary" @click="open()">＋ 添加关键词</button><span class="avatar">{{String(session.user?.name||session.user?.username||'DZ').slice(0,2).toUpperCase()}}</span></div>
    </header>
