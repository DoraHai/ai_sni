...
import { collectSeoGscTraffic, collectSeoOverviewMetrics, crawlSeoSite, createSeoKeyword, fetchSeoAutomationRuns, fetchSeoCrawlRuns, fetchSeoGscConnection, fetchSeoOverview, testSeoGscConnection, updateSeoGscConnection } from '../../api/seo'
...
const automationStatus = (run) => {
  ...
  return ({ completed: '已完成', partial: '部分成功', failed: '失败', running: '运行中' }[run.status] || run.status)
}
...
async function loadAutomation() {
  ...
  automationError.value = ''
  try { automation.value = await fetchSeoAutomationRuns({ tenantId: currentTenantId.value, limit: 30 }) }
  catch (e) { automationError.value = e.message }
...
watch(engine, load)
watch(siteId, async () => { await load(); await loadGsc() })
...
<template>
  <div class="dashboard" v-loading="loading">
    ...
    <section class="automation-panel">
      <header>
        <div><strong>自动化运行状态</strong><small>当前客户的租户级定时任务汇总</small></div>
      ...
      <div class="automation-grid">
        <article v-for="job in automationJobs" :key="job.key">
          ...
          <time>{{ automationTime(automationRun(job.key)) }}</time>
        </article>
      </div>
    </section>
...
.automation-panel{margin:0 0 14px;border:1px solid #dfe5ee;border-radius:10px;background:#fff;overflow:hidden}.automation-panel>header{padding:12px 17px;border-bottom:1px solid #edf0f4;display:flex;align-items:center;justify-content:space-between;gap:12px}.automation-panel>header strong,.automation-panel>header small{display:block}.automation-panel>header strong{font-size:12px}.automation-panel>header small{margin-top:3px;color:#8a94a4;font-size:9px}.automation-panel>header button{padding:5px 9px;border:1px solid #dce3ee;border-radius:6px;background:#fff;color:#2864e8;font-size:9px;cursor:pointer}.automation-panel>header button:disabled{opacity:.5;cursor:not-allowed}.automation-grid{display:grid;grid-template-columns:repeat(3,1fr)}.automation-grid article{padding:13px 17px;border-right:1px solid #edf0f4;display:grid;grid-template-columns:1fr auto;gap:6px 12px;align-items:center}.automation-grid article:last-child{border-right:0}.automation-grid b,.automation-grid small{display:block}.automation-grid b{font-size:11px}.automation-grid small,.automation-grid span,.automation-grid time{color:#8994a5;font-size:9px}.automation-grid em{padding:3px 7px;border-radius:999px;background:#f2f4f7;color:#8b95a4;font-size:9px;font-style:normal}.automation-grid em.completed{background:#e8f7ef;color:#138b58}.automation-grid em.partial,.automation-grid em.running{background:#fff4df;color:#ac7020}.automation-grid em.failed{background:#fff0ee;color:#d85145}.automation-grid span{grid-column:1/-1}.automation-grid time{grid-column:1/-1}@media(max-width:800px){.automation-grid{grid-template-columns:1fr}.automation-grid article{border-right:0;border-bottom:1px solid #edf0f4}.automation-grid article:last-child{border-bottom:0}}
