...
import { collectSeoGscTraffic, collectSeoOverviewMetrics, crawlSeoSite, createSeoKeyword, fetchSeoAutomationRuns, fetchSeoCrawlRuns, fetchSeoGscConnection, fetchSeoOverview, testSeoGscConnection, triggerSeoAutomationRun, updateSeoGscConnection } from '../../api/seo'
...
const automationTriggering = ref('')
...
const gsc = ref({ configured: false, enabled: false, property_url: '', service_account_email: null })
const gscForm = reactive({ property_url: '', enabled: true })
const data = ref({ stats: {}, opportunities: [], page_issues: [], collection_status: [], trend: [], tasks: [] })
const automation = ref({ items: [], latest_by_job: {} })
let automationLoadSequence = 0
...
const canOperateAutomation = computed(() => !session.isLoggedIn || session.canEdit('seo.dashboard'))
const selectedSiteName = computed(() => sites.value.find((item) => item.id === siteId.value)?.name || '当前网站')
...
const automationRun = (jobType) => automation.value.latest_by_job?.[jobType] || null
const automationStatus = (run) => {
  if (!run) return '尚无运行记录'
  if (run.stale) return '运行超时'
  return ({ queued: '排队中', completed: '已完成', partial: '部分成功', failed: '失败', running: '运行中' }[run.status] || run.status)
}
const automationStatusClass = (run) => run?.stale ? 'failed' : (run?.status || 'empty')
const automationProgress = (run) => {
  if (!run) return '等待首次定时运行'
  if (!run.planned_count) return `成功 ${fmt(run.success_count)} · 失败 ${fmt(run.failed_count)}`
  return `成功 ${fmt(run.success_count)} / ${fmt(run.planned_count)} · 失败 ${fmt(run.failed_count)} · 跳过 ${fmt(run.skipped_count)}`
}
const automationTime = (run) => run?.completed_at || run?.started_at
  ? new Date(run.completed_at || run.started_at).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' })
  : '—'
const automationMeta = (run) => run
  ? `${run.trigger_type === 'manual' ? '手动' : '定时'}${run.requested_by ? ` · 操作人 #${run.requested_by}` : ''} · ${automationTime(run)}`
  : '—'
const automationBusy = (jobType) => (automation.value.items || []).some(
  (run) => run.job_type === jobType && !run.stale && ['queued', 'running'].includes(run.status),
)
...
async function loadAutomation() {
  const sequence = ++automationLoadSequence
  if (!currentTenantId.value) {
    automation.value = { items: [], latest_by_job: {} }
  ...
  try {
    const result = await fetchSeoAutomationRuns({ tenantId: requestedTenantId, siteId: requestedSiteId, limit: 30 })
    if (currentTenantId.value === requestedTenantId && siteId.value === requestedSiteId) automation.value = result
  } catch (e) {
    if (currentTenantId.value === requestedTenantId && siteId.value === requestedSiteId) automationError.value = e.message
  }
  finally { if (sequence === automationLoadSequence) automationLoading.value = false }
}

async function triggerAutomation(job) {
  if (!siteId.value) return ElMessage.warning('请先选择 SEO 网站')
  const requestedTenantId = currentTenantId.value
  const requestedSiteId = siteId.value
  const requestedSiteName = selectedSiteName.value
  try {
    await ElMessageBox.confirm(
      `仅重跑「${requestedSiteName}」的${job.label}；不调用 AI、不发布内容、不修改客户网站。确定继续？`,
      '手动重跑确认',
  ...
  if (currentTenantId.value !== requestedTenantId || siteId.value !== requestedSiteId) {
    return ElMessage.warning('客户或网站已切换，请重新确认')
  }
  automationTriggering.value = job.key
  try {
    const result = await triggerSeoAutomationRun({
      tenant_id: requestedTenantId,
      site_id: requestedSiteId,
      job_type: job.key,
    })
    if (result.run && currentTenantId.value === requestedTenantId && siteId.value === requestedSiteId) {
      automation.value = {
        ...automation.value,
        items: [result.run, ...(automation.value.items || [])],
        latest_by_job: { ...automation.value.latest_by_job, [job.key]: result.run },
      }
    }
    ElMessage.success(`${job.label}已进入后台队列，请稍后手动刷新状态`)
  } catch (e) { ElMessage.error(e.message) }
  finally { automationTriggering.value = '' }
}

async function loadSites() {
  if (!currentTenantId.value) { sites.value = []; siteId.value = null; await Promise.all([load(), loadAutomation()]); return }
  try {
    ...
    if (nextSiteId !== siteId.value) { siteId.value = nextSiteId; await loadAutomation(); return }
  } catch (e) {
...
async function loadGsc() {
  ...
  try {
    gsc.value = await fetchSeoGscConnection({ tenantId: currentTenantId.value, siteId: siteId.value })
    Object.assign(gscForm, { property_url: gsc.value.property_url || '', enabled: gsc.value.enabled !== false })
  } catch (e) { error.value = e.message }
}

async function saveGsc() {
  if (!gscForm.property_url.trim()) return ElMessage.warning('请填写 Search Console property')
  gscBusy.value = true
  try {
    gsc.value = await updateSeoGscConnection({ tenant_id: currentTenantId.value, site_id: siteId.value, property_url: gscForm.property_url.trim(), enabled: gscForm.enabled })
    gscDialog.value = false
    ElMessage.success('Search Console 网站资产已保存')
  } catch (e) { ElMessage.error(e.message) }
  finally { gscBusy.value = false }
}

async function testGsc() {
  gscBusy.value = true
  try {
    await testSeoGscConnection({ tenant_id: currentTenantId.value, site_id: siteId.value, days: 3 })
    ElMessage.success('Search Console 连接验证通过')
  } catch (e) { ElMessage.error(e.message) }
  finally { gscBusy.value = false }
}

async function collectGsc() {
  if (!gsc.value.enabled) { gscDialog.value = true; return }
  gscBusy.value = true
  try {
    const result = await collectSeoGscTraffic({ tenant_id: currentTenantId.value, site_id: siteId.value, days: 28 })
    ElMessage.success(`已采集近 28 天真实自然点击 ${Number(result.clicks || 0).toLocaleString('zh-CN')} 次`)
    await load()
  } catch (e) { ElMessage.error(e.message) }
  finally { gscBusy.value = false }
}
...
watch(engine, load)
watch(siteId, async () => { await load(); await loadGsc(); await loadAutomation() })
...
<template>
  <div class="dashboard" v-loading="loading">
    ...
    <section class="automation-panel">
      <header>
        <div><strong>自动化运行状态</strong><small>当前客户的定时汇总与所选网站的手动记录</small></div>
        <button type="button" :disabled="automationLoading" @click="loadAutomation">{{ automationLoading ? '刷新中…' : '刷新状态' }}</button>
      </header>
      <el-alert v-if="automationError" :title="automationError" type="warning" :closable="false" />
      <div class="automation-grid">
        <article v-for="job in automationJobs" :key="job.key">
          <div><b>{{ job.label }}</b><small>{{ job.schedule }}</small></div>
          <em :class="automationStatusClass(automationRun(job.key))">{{ automationStatus(automationRun(job.key)) }}</em>
          <span>{{ automationProgress(automationRun(job.key)) }}</span>
          <time>{{ automationMeta(automationRun(job.key)) }}</time>
          <button
            v-if="canOperateAutomation"
            type="button"
            :disabled="!siteId || automationTriggering === job.key || automationBusy(job.key)"
            @click="triggerAutomation(job)"
          >{{ automationTriggering === job.key ? '提交中…' : automationBusy(job.key) ? '运行中' : '手动重跑' }}</button>
        </article>
      </div>
    </section>
    ...
    <section class="site-metric-grid">
      ...
      <article>
        ...
        <div class="metric-actions"><button @click="gscDialog = true">配置 GSC</button><button :disabled="gscBusy" @click="collectGsc">采集近 28 天</button></div>
      </article>
      <article>
        ...
        <small v-if="data.crawl?.status === 'failed' && data.crawl?.error_summary" class="crawl-error">失败原因：{{ data.crawl.error_summary }}</small>
      </article>
    </section>
    ...
    <el-dialog v-model="gscDialog" title="接入 Google Search Console" width="620px">
      <el-alert v-if="!gsc.configured" title="服务器尚未配置 GSC 服务账号；可先保存网站资产，配置密钥后再验证和采集。" type="warning" :closable="false" />
      <el-form label-position="top" class="gsc-form">
        <el-form-item label="Search Console property"><el-input v-model="gscForm.property_url" placeholder="sc-domain:example.com 或 https://www.example.com/" /></el-form-item>
        <el-form-item label="启用采集"><el-switch v-model="gscForm.enabled" /></el-form-item>
        <el-form-item v-if="gsc.service_account_email" label="需要添加为资产用户的服务账号"><el-input :model-value="gsc.service_account_email" readonly /></el-form-item>
      </el-form>
      <template #footer><el-button @click="gscDialog = false">取消</el-button><el-button :disabled="!gsc.enabled || !gsc.configured" :loading="gscBusy" @click="testGsc">验证连接</el-button><el-button type="primary" :loading="gscBusy" @click="saveGsc">保存</el-button></template>
    </el-dialog>
  </div>
</template>
...
.automation-panel{margin:0 0 14px;border:1px solid #dfe5ee;border-radius:10px;background:#fff;overflow:hidden}.automation-panel>header{padding:12px 17px;border-bottom:1px solid #edf0f4;display:flex;align-items:center;justify-content:space-between;gap:12px}.automation-panel>header strong,.automation-panel>header small{display:block}.automation-panel>header strong{font-size:12px}.automation-panel>header small{margin-top:3px;color:#8a94a4;font-size:9px}.automation-panel>header button,.automation-grid article>button{padding:5px 9px;border:1px solid #dce3ee;border-radius:6px;background:#fff;color:#2864e8;font-size:9px;cursor:pointer}.automation-panel button:disabled{opacity:.5;cursor:not-allowed}.automation-grid{display:grid;grid-template-columns:repeat(3,1fr)}.automation-grid article{padding:13px 17px;border-right:1px solid #edf0f4;display:grid;grid-template-columns:1fr auto;gap:6px 12px;align-items:center}.automation-grid article:last-child{border-right:0}.automation-grid b,.automation-grid small{display:block}.automation-grid b{font-size:11px}.automation-grid small,.automation-grid span,.automation-grid time{color:#8994a5;font-size:9px}.automation-grid em{padding:3px 7px;border-radius:999px;background:#f2f4f7;color:#8b95a4;font-size:9px;font-style:normal}.automation-grid em.completed{background:#e8f7ef;color:#138b58}.automation-grid em.partial,.automation-grid em.running,.automation-grid em.queued{background:#fff4df;color:#ac7020}.automation-grid em.failed{background:#fff0ee;color:#d85145}.automation-grid span{grid-column:1/-1}.automation-grid time{grid-column:1}.automation-grid article>button{grid-column:2;grid-row:4;justify-self:end}@media(max-width:800px){.automation-grid{grid-template-columns:1fr}.automation-grid article{border-right:0;border-bottom:1px solid #edf0f4}.automation-grid article:last-child{border-bottom:0}}
.metric-actions{display:flex;gap:7px;margin-top:10px}.metric-actions button{padding:4px 7px;border:1px solid #dce3ee;border-radius:6px;background:#fff;color:#2864e8;font-size:9px;cursor:pointer}.metric-actions button:disabled{opacity:.5;cursor:not-allowed}
.site-metric-grid .crawl-error{display:block;margin-top:8px;color:#d85145;overflow-wrap:anywhere}
</style>
