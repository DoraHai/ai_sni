...
import { createSeoKeyword, fetchSeoOverview } from '../../api/seo'
...
async function load() {
  ...
  loading.value = true
  error.value = ''
  try { data.value = await fetchSeoOverview({ tenantId: currentTenantId.value, engine: engine.value }) }
...
watch([engine, currentTenantId], load)
onMounted(load)
</script>
  ...
  <div class="dashboard" v-loading="loading">
    ...
    <section class="dashboard-grid">
      <article class="trend-card">
        ...
        <footer><div><span>当前 Top 10</span><b>{{ fmt(stats.top10) }} 个</b></div><div><span>排名覆盖</span><b>{{ fmt(stats.ranked) }} / {{ fmt(stats.keywords) }}</b></div><div><span>连续下降关键词</span><b class="danger">{{ fmt(stats.rank_anomalies) }} 个</b></div></footer>
      </article>

      <article class="priority-card">
        <header><div><h2>今日优先级</h2><p>按流量机会与风险综合排序</p></div><span>{{ tasks.length }} 项</span></header>
        <div v-if="tasks.length" class="task-list">
          <button v-for="(task, index) in tasks" :key="task.type" @click="router.push(task.path)">
