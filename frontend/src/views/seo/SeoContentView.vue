...
const form = reactive({ title: '', keyword_id: null, content_type: 'article', outline: '', draft: '', humanized_content: '', status: 'planned', page_url: '', author: '' })
...
const items = computed(() => {
  ...
  return baseItems.value.filter((item) => {
    ...
    return statusMatch && (!needle || item.title.toLowerCase().includes(needle) || keyword.toLowerCase().includes(needle))
  })
})
...
const keywordFor = (row) => keywords.value.find((item) => item.id === row.keyword_id)?.keyword || ''
...
async function save() {
  ...
  saving.value = true
  try {
    const payload = { ...form, outline: form.outline || null, draft: form.draft || null, humanized_content: form.humanized_content || null, page_url: form.page_url || null, author: form.author || null, published_at: form.status === 'published' ? new Date().toISOString() : null }
  ...
  <div class="content-prototype" v-loading="loading">
          ...
          <table class="content-table">
            ...
            <tbody>
              <tr v-for="row in items" :key="row.id">
                ...
                <td><span v-if="keywordFor(row)" class="keyword-tag">{{ keywordFor(row) }}</span><span v-else class="muted-text">待绑定</span></td>
                <td>
    ...
    <el-dialog v-model="dialog" :title="editing ? `编辑${config.title}` : config.button" width="820px">
      <el-form label-position="top" class="suite-form">
        ...
        <el-form-item label="目标关键词"><el-select v-model="form.keyword_id" clearable filterable><el-option v-for="item in keywords" :key="item.id" :label="item.keyword" :value="item.id" /></el-select></el-form-item>
