  ...
  <div v-loading="loading" class="geo-page">
    <div class="page-header">
      <div>
        ...
        <div class="page-desc">
          对比两个观测窗口的关键可见性指标变化（百分点）。「未测」表示该窗无足够样本，不会用 0 冒充。
        </div>
      </div>
    ...
    <section class="geo-panel">
      ...
      <div class="geo-filter-bar">
        <div>
          <div class="ctrl-label">对比前（Before）</div>
          <el-date-picker
            v-model="beforeRange"
            type="daterange"
        ...
        <div>
          <div class="ctrl-label">对比后（After）</div>
          <el-date-picker
            v-model="afterRange"
            type="daterange"
            value-format="YYYY-MM-DD"
    ...
    <section v-if="result" class="geo-panel">
      ...
      <p class="geo-metric-note">
        对比前快照 {{ fmtInt(result.before?.snapshots_total) }} ·
        对比后快照 {{ fmtInt(result.after?.snapshots_total) }} ·
        Δ 单位为百分点（pp）
      </p>
    ...
    <div v-else-if="!loading && !error" class="geo-empty">
      <div class="empty-title">选择窗口后点击「重新计算」</div>
