...
async function submitCreate() {
  ...
  try {
    ...
    await load()
    router.push(`/geo/articles/${task.id}`)
  } catch (e) {
...
function openEditor(row) {
  router.push(`/geo/articles/${row.id}`)
}

function openDistribution(row) {
  router.push(`/geo/articles/${row.id}/distribution`)
}

function openCitations(row) {
  router.push({ path: '/geo/sources', query: { task_id: String(row.id) } })
}
...
<template>
  ...
  >
    <template #actions>
      <router-link class="gd-btn" to="/geo/media">信源素材库</router-link>
