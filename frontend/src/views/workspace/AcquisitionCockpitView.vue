...
const demoMode = computed(() => session.user?.username === 'workbench_test_readonly' && Number(session.tenantId) === 16)
const liveClock = computed(() => new Intl.DateTimeFormat('zh-CN', {
  timeZone: 'Asia/Shanghai', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false,
}).format(now.value))
const liveEvents = computed(() => {
  const signal = cards.value.filter(item => Number(item.urgentCount) > 0).slice(0, 4).map((item, index) => ({
    id: `metric-${item.id}`,
    time: displayTime(lastReadAt.value),
    module: item.moduleCode,
    title: `${item.label}出现 ${item.urgentCount} 项待处理`,
    metricId: item.id,
  }))
  if (signal.length || !demoMode.value) return signal
  return [
    { id: 'demo-sem', time: '14:32', module: 'sem', title: '高成本词簇进入重点观察', metricId: cards.value.find(item => item.moduleCode === 'sem')?.id },
    { id: 'demo-geo', time: '14:28', module: 'geo', title: '新一轮 AI 回答探测已完成', metricId: cards.value.find(item => item.moduleCode === 'geo')?.id },
    { id: 'demo-seo', time: '14:06', module: 'seo', title: '核心页面检查结果已更新', metricId: cards.value.find(item => item.moduleCode === 'seo')?.id },
...
function focusEvent(item) { if (item.metricId) focusMetric(item.metricId) }
...
<template>
  <main ref="shellEl" class="cockpit-shell" :class="[`mode-${viewMode}`, { 'is-fullscreen': fullscreen }]" v-loading="loading">
    ...
    <header class="command-bar">
      ...
      <div class="live-badge" :class="{ replay: demoMode }"><i></i><span>{{ demoMode ? '演示回放' : 'LIVE' }}</span><b>{{ liveClock }}</b><small>数据 {{ displayTime(lastReadAt) }}</small></div>
    ...
    <section class="pulse-strip" aria-label="工作台实时状态">
      ...
      <div><span>读取完成</span><strong>{{ readyModules }}<small>/{{ availableModules.length }}</small></strong><em :style="{ '--progress': `${readProgress}%` }"></em></div>
      <div :class="{ urgent: urgentItems }"><span>系统标记事项</span><strong>{{ urgentItems }}</strong><small>{{ urgentItems ? '请优先查看行动台账' : '暂无已识别事项' }}</small></div>
      <div><span>可核对指标</span><strong>{{ cards.length }}</strong><small>均可打开查看来源</small></div>
      <div :class="{ caution: boundaryItems }"><span>边界待说明</span><strong>{{ boundaryItems }}</strong><small>未完成、部分或暂不可用</small></div>
    </section>
    ...
    <section class="operations-grid">
      ...
      <div v-if="viewMode !== 'chat'" class="data-stage">
        ...
        <div v-if="activeSection === 'dashboard'" class="tactical-map" :class="`focus-${activeModule}`">
          <div class="target-core"><small>当前目标</small><strong>{{ activeModule === 'all' ? '全域获客效率' : `${activeModule.toUpperCase()} 战线` }}</strong><span>{{ highlightedMetricIds.length ? `已锁定 ${highlightedMetricIds.length} 项证据` : '选择战线或向 DeepSeek 下达指令' }}</span></div>
          <button v-for="(code, index) in ['sem','seo','geo']" :key="code" :class="[`tactical-node`, `node-${code}`, { active: activeModule === code }]" type="button" :style="{ '--node-index': index }" @click="activeModule = activeModule === code ? 'all' : code">
            <span>{{ code.toUpperCase() }}</span><b>{{ cards.filter(card => card.moduleCode === code).length }}</b><small>{{ moduleStatusLabel(code) }}</small>
          </button>
          <svg viewBox="0 0 600 170" aria-hidden="true"><path d="M82 84 C190 20 410 20 518 84"/><path d="M82 84 C210 150 390 150 518 84"/><circle cx="300" cy="85" r="63"/></svg>
        </div>

        <template v-if="activeSection === 'dashboard'">
          <div class="battle-layout">
          <div v-if="filteredCards.length" class="metric-grid" :class="{ compact: compactCards }" role="tabpanel" :aria-label="activeModule === 'all' ? '全域指标' : `${activeModule.toUpperCase()} 指标`">
            <MetricEvidenceCard v-for="card in filteredCards" :key="card.id" :metric="card" :context-revision="viewState.revision" :highlighted="highlightedMetricIds.includes(card.id)" @focus="focusMetric" @discuss="discuss" @retry="loadAll" />
          </div>
          <div v-else class="data-empty"><i></i><strong>当前范围尚无可展示数字</strong><span>系统正在核对模块、权限与业务对象，不会显示演示值。</span></div>
          <aside class="event-feed"><header><div><small>EVENT STREAM</small><strong>战况事件</strong></div><button type="button" @click="feedPaused = !feedPaused">{{ feedPaused ? '继续' : '暂停' }}</button></header><div v-if="liveEvents.length" class="event-list"><button v-for="item in liveEvents" :key="item.id" :class="`event-${item.module}`" type="button" @click="focusEvent(item)"><time>{{ item.time }}</time><i></i><span>{{ item.title }}</span><b>定位 ↗</b></button></div><p v-else>当前没有新的异常事件</p><footer><span :class="{ paused: feedPaused }"></span>{{ feedPaused ? '事件流已暂停' : (demoMode ? '正在播放演示事件' : '等待真实数据变化') }}</footer></aside>
          </div>
        </template>
