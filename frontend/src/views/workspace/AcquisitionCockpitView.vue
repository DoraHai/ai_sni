  ...
  <main ref="shellEl" class="cockpit-shell" :class="[`mode-${viewMode}`, { 'is-fullscreen': fullscreen }]" v-loading="loading">
    ...
    <section class="operations-grid">
      ...
      <div v-if="viewMode !== 'chat'" class="data-stage">
        ...
        <div v-if="activeSection === 'dashboard'" class="channel-control-strip">
          <div class="control-summary">
            <small>当前查看</small>
            <strong>{{ activeModule === 'all' ? '全域获客数据' : `${activeModule.toUpperCase()} 数据` }}</strong>
            <span>{{ highlightedMetricIds.length ? `已选中 ${highlightedMetricIds.length} 项指标` : '点击通道筛选，点击指标查看依据' }}</span>
          </div>
          <div class="channel-controls" role="group" aria-label="获客数据通道">
            <button v-for="code in ['sem','seo','geo']" :key="code" :class="[`channel-control`, `channel-${code}`, { active: activeModule === code }]" type="button" @click="activeModule = activeModule === code ? 'all' : code">
              <span><i></i>{{ code.toUpperCase() }}</span>
              <b>{{ cards.filter(card => card.moduleCode === code).length }}<small> 项指标</small></b>
              <em>{{ moduleStatusLabel(code) }}</em>
            </button>
          </div>
          <button v-if="activeModule !== 'all'" class="show-all-control" type="button" @click="activeModule = 'all'">查看全部</button>
        </div>
