...
const activeOptionId = computed(() => selectedItem.value
  ? `visual-${visualId}-${props.visualization.type}-${props.visualization.type === 'trend' ? 'current' : selectedItem.value.key}`
  : undefined)
...
function scrubTrend(event, activate = false) {
  const rect = event.currentTarget.getBoundingClientRect()
  const index = visualizationIndexFromPointer(event.clientX, rect.left, rect.width, points.value.length)
  if (index >= 0) select(index, activate)
}
...
<style scoped>
.metric-visual{position:relative;margin-top:10px;padding:4px;border-radius:9px;color:#9daec0}.metric-visual:focus-visible{outline:2px solid #8ce2f1;outline-offset:2px}.trend-scrubber{touch-action:pan-y;cursor:crosshair}.trend{display:block;width:100%;height:90px;overflow:visible}.baseline{stroke:#29384b;fill:none}.trend-line{stroke:#67ddca;stroke-width:2.2;fill:none;stroke-linecap:round;stroke-linejoin:round}.trend-point{fill:#67ddca}.coverage,.rate-copy,.visual-readout,.empty-visual{margin:6px 0 0;font-size:10px;line-height:1.45}.coverage-partial,.rate-copy{color:#e5ba70}.empty-visual{min-height:55px;display:grid;place-items:center}.funnel-stages{display:grid;gap:7px}.funnel-stages button{position:relative;display:grid;grid-template-columns:1fr auto;gap:8px;overflow:hidden;padding:9px 10px;border:1px solid #2c4558;border-radius:9px;background:#0d1d2a;color:#c8d8e3;text-align:left;cursor:pointer}.funnel-stages button i{position:absolute;left:0;bottom:0;height:2px;background:#67ddca}.funnel-stages button:nth-child(2) i{background:#6f9ee8}.funnel-stages span{font-size:10px}.funnel-stages b{font-size:11px}.distribution{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:4px 10px}.distribution button{display:grid;grid-template-columns:minmax(48px,70px) 1fr 28px;align-items:center;gap:5px;min-height:44px;padding:3px 0;border:0;background:transparent;color:#a9bdca;text-align:left;cursor:pointer}.distribution span,.distribution b{overflow:hidden;font-size:9px;text-overflow:ellipsis;white-space:nowrap}.distribution b{text-align:right}.distribution i{height:5px;border-radius:4px;background:#253849;overflow:hidden}.distribution em{display:block;height:100%;border-radius:inherit;background:#6f9ee8}.metric-visual button[aria-selected=true]{color:#eafffb}.visual-readout{color:#d4e8ef}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}@media(prefers-reduced-motion:reduce){*{transition:none!important}}
</style>
