...
const hasVisualization = computed(() => Object.hasOwn(metric.value, 'visualization'))
const visualization = computed(() => validVisualization(metric.value.visualization) ? metric.value.visualization : null)
const invalidVisualization = computed(() => hasVisualization.value && !visualization.value)
...
function selectVisualization(item) { emit('focus', item.metricId || metric.value.id) }
...
<template>
  <article class="evidence-card" :class="[`is-${metric.state}`, { 'is-highlighted': highlighted }]" :aria-busy="metric.state === 'loading'">
    ...
    <MetricVisualization v-if="visualization" :visualization="visualization" :metric-label="metric.label" @select="selectVisualization" />
    <p v-else-if="invalidVisualization" class="empty-trend">图形数据无法核验，暂不展示。</p>
