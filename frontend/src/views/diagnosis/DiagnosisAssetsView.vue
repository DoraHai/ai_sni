...
const props = defineProps({
  ...
  initialBrand: { type: Object, default: null },
})
...
// Competitors are no longer managed in the brand-foundation UI. Keep any
// previously saved values as an opaque pass-through so editing the profile
...
async function loadProfile() {
  ...
  if (activePage.value === 'brand') {
    const sessionBrand = props.initialBrand || {}
    Object.assign(brand, {
      name: sessionBrand.name || '',
      website: sessionBrand.website || props.initialWebsite || '',
      industry: sessionBrand.industry || '',
      business_desc: sessionBrand.business_desc || '',
      brand_terms: joinLines(sessionBrand.brand_terms),
      core_products: joinLines(sessionBrand.core_products),
      proof_points: joinLines(sessionBrand.proof_points),
    })
    savedCompetitors.value = []
    discoveryEvidence.value = {}
    discoveryUsedAI.value = false
    discoverError.value = ''
    profileReady.value = Boolean(props.initialBrand)
...
async function discoverProfile() {
  ...
  try {
    ...
    ElMessage.success('官网品牌信息已整理，请确认识别结果')
  } catch (error) {
  ...
  try {
    if (activePage.value === 'brand') {
      ...
      const result = await saveGeoBrand({
        ...
        competitors: savedCompetitors.value,
      })
...
watch(() => props.initialWebsite, () => {
  if (activePage.value !== 'brand') return
  profileLoaded.value = false
  activate()
})
onMounted(activate)
      ...
      <section class="form-card">
        ...
        <div class="website-starter">
          <div>
            ...
            <p><strong>AI 品牌提取及抽样</strong><small>读取官网公开事实，生成可确认的品牌基础信息。</small></p>
          </div>
          <label>
        ...
        <div v-if="Object.keys(discoveryEvidence).length" class="review-note">
          ...
          <p><strong>请确认自动识别结果</strong><small>品牌字段来自官网公开信息，请确认后再保存为当前域名的诊断档案。</small></p>
        </div>
        <footer>
...
.review-note small{font-size:11px}.review-note strong{font-size:13px}
.form-card>footer span{font-size:12px}.form-card button,.composer button,.knowledge-tools button,.empty button{font-size:13px}
@media(max-width:850px){.completion-strip{grid-template-columns:1fr}.form-card .quick-start{width:100%}}
</style>
