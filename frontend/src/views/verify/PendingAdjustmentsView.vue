...
import { fetchPendingAdjustments, genAiVerdict, markVerified } from '../../api/adjustmentVerify'
...
const DIR_META = { raise: { label: '加价', cls: 'd-up' }, lower: { label: '降价', cls: 'd-down' }, flat: { label: '调整', cls: '' } }
...
async function load() {
  ...
  error.value = ''
  try {
    data.value = await fetchPendingAdjustments({ tenantId: TENANT_ID.value, days: days.value, status: statusFilter.value })
  } catch (e) {
...
// 调后 vs 调前的变化方向（用于上色：排名越小越好、消费降通常好）
...
async function runAi(item) {
  ...
  try {
    ...
    else { ElMessage.success('AI 研判完成'); load() }
  } catch (e) {
...
watch([TENANT_ID, days, statusFilter], load)
onMounted(load)
</script>
  ...
  <div v-loading="loading">
    <div class="page-header">
      <div>
        ...
        <div class="page-desc">近 {{ days }} 天的出价调整 · 对比调价前后效果 + AI 研判是否达成目的 · 核对后标记已验证</div>
      </div>
    </div>
    ...
    <div class="toolbar">
      ...
      <span v-if="data" class="summary">共 {{ data.summary.total }} · 待验证 <b>{{ data.summary.pending }}</b> · 已验证 {{ data.summary.verified }}</span>
    </div>

    <div v-for="it in data?.items || []" :key="it.dedup_key" class="adj-card" :class="{ verified: it.review.status === 'verified' }">
      <div class="adj-head">
        ...
        <span class="dir" :class="DIR_META[it.direction].cls">{{ DIR_META[it.direction].label }}</span>
      ...
      <table class="eff">
        <thead><tr><th></th><th>日均消费</th><th>日均点击</th><th>点击率</th><th>平均排名</th></tr></thead>
        <tbody>
    ...
    <div v-if="!loading && !(data?.items || []).length" class="empty">近 {{ days }} 天没有出价调整记录</div>
  </div>
</template>
