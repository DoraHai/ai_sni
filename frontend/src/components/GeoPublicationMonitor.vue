...
async function load() {
  const run = ++epoch
  rows.value = []; error.value = ''; busy.value = false; loading.value = false; monitoringActive.value = true
  if (!props.tenantId || !props.taskId) return
  loading.value = true
  try {
    const data = await listGeoPublicationMonitor(props.taskId, props.tenantId)
    if (run === epoch) { rows.value = data.items || []; monitoringActive.value = data.monitoring_active !== false }
  } catch (e) { if (run === epoch) error.value = e.message || '监测记录读取失败' }
  finally { if (run === epoch) loading.value = false }
}
async function check(row) {
  if (busy.value || loading.value || !monitoringActive.value || row.can_check === false) return
  const run = epoch
...
<template>
  <section class="geo-publication-monitor">
    ...
    <p v-if="!monitoringActive">内容已归档或取消，监测已停止；下方保留历史记录。</p>
    <p v-if="loading">正在读取发布监测记录…</p>
    <p v-if="monitoringActive && !loading && !rows.length && !error">真实发布登记后，自动开始监测。</p>
    <article v-for="row in rows" :key="row.publication_id">
      <strong>#{{ row.publication_id }} · {{ labels[row.state] || row.state }}</strong>
      <p>{{ row.url || '发布记录缺少网址，无法检查' }}</p>
      ...
      <p v-if="row.last_error">最近一次检查未完成，稍后自动重试；下方结果仍是此前观测。</p>
      <button type="button" :disabled="busy || !monitoringActive || row.can_check === false" @click="check(row)">{{ busy ? '检查中…' : '重新检查' }}</button>
