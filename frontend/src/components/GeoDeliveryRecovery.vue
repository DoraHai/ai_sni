...
<template>
  <section class="gd-card recovery">
    ...
    <p v-if="!rows.length && !error">暂无发送记录。</p>
    <article v-for="row in rows" :key="row.delivery_key" class="delivery">
      <strong>{{ row.channel }} · 账号 #{{ row.account_id }} · {{ row.mode === 'draft' ? '草稿' : '发布' }} · {{ labels[row.state] || row.state }}</strong>
      <p>最近更新：{{ row.updated_at }}。发送中记录需等待至少 10 分钟才可恢复。</p>
      <template v-if="['sending', 'unknown', 'failed'].includes(row.state)">
        <el-input v-model="row.note" :disabled="busy" type="textarea" maxlength="1000" placeholder="核对说明（至少 10 字，请勿填写账号密码）" aria-label="核对说明" />
        <div v-if="row.mode === 'publish'" class="recovery-actions">
        ...
        <el-checkbox v-model="row.confirmed" :disabled="busy">我已核对渠道后台，确认没有生成对应文章或草稿</el-checkbox>
        <el-button :disabled="busy || !row.confirmed || row.note.trim().length < 10" @click="resolve(row, 'allow_retry')">允许重新尝试</el-button>
      </template>
