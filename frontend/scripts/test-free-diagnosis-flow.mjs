...
await test('报告布局与流程保持不变，只允许三态修正和独立打印接入',()=>{
  ...
  // Enumerate the permitted state expressions. All other report markup stays protected.
  const stateChanges = [
    ['Math.max(0, findings.length - passedCount)', 'problems.length'],
    ['/\{\{ findings.length \}\}', '/\{\{ evaluatedFindings.length \}\}'],
    ['Math.max(findings.length, 1)', 'Math.max(evaluatedFindings.length, 1)'],
    ['{{ item.score }}', "{{ item.score ?? '未检测' }}"],
    ['{ failed: !item.passed }', '{ failed: item.passed === false }'],
    ["{{ item.passed ? '✓' : '!' }}", "{{ item.passed == null ? '—' : item.passed ? '✓' : '!' }}"],
    [":class=\"item.passed ? 'passed' : item.severity\"", ":class=\"item.passed == null ? '' : item.passed ? 'passed' : item.severity\""],
  ]
  const corrected = stateChanges.reduce((value,[from,to]) => value.replaceAll(from,to), expected)
  assert.equal(content(after),corrected)
  assert.ok(after.includes('<DiagnosticPrintReport v-if="audit"'))
  assert.ok(!after.includes('node.open = true'))
  const directions = after.slice(after.indexOf('<div v-if="priorityDirections.length"'), after.indexOf('<div v-else class="priority-clean-state">'))
  assert.ok(!directions.includes('<button'))
  assert.ok(!directions.includes('项当前问题'))
  assert.ok(after.includes('href="/deal-sniper/portal">获取完整优化方案'))
...
await test('导出等待异步 AI 与性能结果，保留真实零值和完整回答', async () => {
  const sample = deferred(), performance = deferred()
  const t = setup({runDeepSeekSample:()=>sample.promise, fetchPageSpeedInsights:()=>performance.promise})
  t.state.audit.value = record()
  let done = false
  const ready = t.flow.waitForReportData().then(value => { done = true; return value })
  await flush(); assert.equal(done, false)
  sample.resolve({snapshot:{ai_sampling:{results:[{question:'问题',response:'完整回答',mentioned:false}]}}})
  await flush(); assert.equal(done, false)
  performance.resolve({status:'available',performance_score:50,metrics:{lcp:{value:13.5,unit:'s'},cls:{value:0}}})
  assert.equal(await ready,true)
  assert.equal(t.state.pageSpeed.value.performance_score,50)
  assert.equal(t.state.pageSpeed.value.metrics.cls.value,0)
  assert.equal(t.state.audit.value.snapshot.ai_sampling.results[0].response,'完整回答')
  t.scope.stop()
})
await test('附加检测失败仍可导出已有数据，切换诊断取消旧导出', async () => {
  const sample = deferred(), performance = deferred()
  const t = setup({runDeepSeekSample:()=>sample.promise, fetchPageSpeedInsights:()=>performance.promise})
  t.state.audit.value = record()
  const ready = t.flow.waitForReportData(); await flush()
  sample.reject(new Error('抽样超时')); performance.resolve({status:'unavailable',reason:'不可用'})
  assert.equal(await ready,true)
  assert.equal(t.flow.statuses.sample,'error')
  t.state.samplingLoading.value = true
  const cancelled = t.flow.waitForReportData(); await flush()
  t.state.audit.value = null
