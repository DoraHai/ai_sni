...
client.defaults.adapter=async config=>{
 ...
 let data
 if(method==='get' && path.endsWith('/tasks')) data=config.params.tenant_id===7?[task]:[]
...
const app=createApp({setup:()=>()=>h('main',{class:'geo-dash geo-page',style:'max-width:1000px;margin:24px auto;padding:20px'},[h('h2','仅内存数据，不连接生产'),h('button',{onClick:()=>tenant.value=tenant.value===7?8:7},'切换测试客户'),h(Evidence,{tenantId:tenant.value})])})
