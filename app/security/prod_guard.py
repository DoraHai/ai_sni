...
from app.config import resolve_baidu_live_write_grants
...
def collect_production_issues(settings: "Settings") -> list[str]:
    ...
    if not bool(getattr(settings, "baidu_write_dry_run", True)):
        try:
            live_write_grants = resolve_baidu_live_write_grants(settings)
        except ValueError as exc:
            issues.append(str(exc))
        else:
            if not live_write_grants:
                issues.append(
                    "BAIDU_LIVE_WRITE_GRANTS or one complete legacy tenant/account/scope "
                    "grant is required when BAIDU_WRITE_DRY_RUN=false"
                )
